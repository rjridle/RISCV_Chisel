####################################################################
# Innovus Foundation Flow Code Generator, Tue Jul 21 14:59:07 CDT 2020
...
if {$vars(restore_design)} { restoreDesign DBS/route.enc.dat riscv }
...
if {[file exists RPT/postroute.summary]} {
   exec /bin/mail -s "FF: riscv, postroute completed ([pwd])" < RPT/postroute.summary ryanridley46@gmail.com
} elseif {[file exists RPT/postroute.summary.gz]} {
   exec gunzip -c RPT/postroute.summary.gz | /bin/mail \
      -s "FF: riscv, postroute completed ([pwd])" ryanridley46@gmail.com
} else {
   exec /bin/mail -s "FF: riscv, postroute completed ([pwd])" < /dev/null ryanridley46@gmail.com
}
