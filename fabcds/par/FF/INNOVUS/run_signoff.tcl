####################################################################
# Innovus Foundation Flow Code Generator, Tue Jul 21 15:54:40 CDT 2020
...
if {$vars(restore_design)} { restoreDesign DBS/postroute.enc.dat riscv }
...
if {[file exists RPT/signoff.summary]} {
   exec /bin/mail -s "FF: riscv, signoff completed ([pwd])" < RPT/signoff.summary ryanridley46@gmail.com
} elseif {[file exists RPT/signoff.summary.gz]} {
   exec gunzip -c RPT/signoff.summary.gz | /bin/mail \
      -s "FF: riscv, signoff completed ([pwd])" ryanridley46@gmail.com
} else {
   exec /bin/mail -s "FF: riscv, signoff completed ([pwd])" < /dev/null ryanridley46@gmail.com
}
