...
then
	...
	sed -i "s|${testFilePlaceholder}|${testFile}.x|1" src/main/scala/riscvSingle/$projectFile.scala
        sbt "runMain $projectFile.top"
	sed -i "s|${testFile}.x|${testFilePlaceholder}|1" src/main/scala/riscvSingle/$projectFile.scala	
else
