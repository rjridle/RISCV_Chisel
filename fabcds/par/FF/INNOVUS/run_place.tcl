####################################################################
# Innovus Foundation Flow Code Generator, Tue Jul 21 17:25:34 CDT 2020
...
if {$vars(restore_design)} { restoreDesign DBS/init.enc.dat riscv }
...
if {[file exists RPT/place.summary]} {
   exec /bin/mail -s "FF: riscv, place completed ([pwd])" < RPT/place.summary ryanridley46@gmail.com
} elseif {[file exists RPT/place.summary.gz]} {
   exec gunzip -c RPT/place.summary.gz | /bin/mail \
      -s "FF: riscv, place completed ([pwd])" ryanridley46@gmail.com
} else {
   exec /bin/mail -s "FF: riscv, place completed ([pwd])" < /dev/null ryanridley46@gmail.com
}
